<?php
    ...
    try {
        ...
        foreach($dbh->query("SELECT order_no, logistics FROM iwebshop_order where user_id='" . $uid . "' AND if_del=0 AND negotiate_status <> 0 order by id desc;") as $row) {
            if($row['logistics']=='' || strripos($row['logistics'],'?')){
        ...
        if($uid!=""){
            foreach($dbh->query("SELECT id, order_no, create_time, status, pay_type, pay_status,negotiate_price,negotiate_status,note,logistics FROM iwebshop_order where user_id='" . $uid . "' AND if_del=0 AND negotiate_status <> 0 order by id desc;") as $row) {
                $order = array();
